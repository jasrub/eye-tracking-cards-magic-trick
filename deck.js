...
var Deck = (function () {
  ...
  var poker = {
    ...
    var pokerMagic = {
    deck: function deck(_deck4) {
      console.log("pokerMagic called")
      _deck4.pokerMagic = _deck4.queued(pokerMagic);

      function pokerMagic(next) {
        var cards = _deck4.cards;
        var len = cards.length;
        ...
        cards.slice(-5).reverse().forEach(function (card, i) {
          card.pokerMagic(i, len, function (i) {
            card.setSide('front');
            _deck4.pokerCards.push(card.i)
            if (i === 4) {
              next();
            }
          });
    ...
    card: function card(_card4) {
      ...
      _card4.pokerMagic = function (i, len, cb) {
        var delay = i * 250;
        ...
        _card4.animateTo({
          ...
          x: Math.round((i - 2.05) * (window.screen.width/5-80) * __fontSize / 16),
          y: Math.round(-110 * __fontSize / 16),
          rot: 0,
  ...
  Deck.ease = ease;
  Deck.modules = { bysuit: bysuit, fan: fan, intro: intro, poker: poker, pokerMagic: pokerMagic, shuffle: shuffle, sort: sort, flip: flip };
  Deck.Card = _card;
  Deck.prefix = prefix;
